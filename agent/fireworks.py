"""Escalation client Fireworks AI (OpenAI-compatible).

SEMUA call wajib lewat FIREWORKS_BASE_URL (aturan) & pakai model dari ALLOWED_MODELS
(dibaca runtime, JANGAN hardcode). Input+output token dihitung ke skor → kirim prompt
sepadat mungkin & batasi max_tokens.
"""
...
class Fireworks:
    ...
    def pick_model(self, prefer: Optional[str] = None) -> Optional[str]:
        """Pilih model dari ALLOWED_MODELS. `prefer` = substring match (mis. 'code')."""
        if not self.models:
            return None
        if prefer:
    ...
    def generate(self, prompt: str, system: Optional[str] = None, *,
        ...
        for attempt in range(retries + 1):
            ...
            except Exception as exc:  # noqa: BLE001 - jangan matikan pipeline
                last_exc = exc
                if attempt < retries:
