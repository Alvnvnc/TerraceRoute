"""Eval pipeline agent/ BARU terhadap task set 8-kategori (tasks_v4.jsonl).

Menjalankan Solver PERSIS seperti di container (kode agent/ tak diubah), tapi
dengan backend Ollama menunjuk instance AMD remote (qwen2.5:3b-instruct = model
yang di-ship). Grading:
  - exact / contains_all : deterministik, boundary-aware (tanpa LLM)
  - pytests              : ekstrak kode dari jawaban lalu EKSEKUSI + assert
  - judge                : gemma3:12b (independen, beda keluarga) vs reference

Output: tabel akurasi per kategori + akurasi klasifikasi router + dump JSONL
(hasil mentah utk analisis lanjutan). Ini data yang menentukan ESCALATION_LEVEL.

Jalankan (env WAJIB diset sebelum import agent.config):
...
# Default env dev → remote AMD (boleh dioverride dari shell).
...
def _norm_match(answer: str, draft: str) -> bool:
    """Cocokkan `answer` di `draft` boundary-aware (porting eval/grader.py, tanpa app/)."""
    a = answer.strip()
    d = draft.lower()
    if _NUMERIC.match(a):
...
def grade_judge(task_prompt: str, draft: str, reference: str) -> bool:
    """Judge independen gemma3:12b via Ollama remote (retry ada di post_json)."""
    prompt = (
...
def main() -> None:
    ...
    for t in tasks:
        t0 = time.time()
        # Retry-on-empty: proxy Jupyter kadang balas HTML transien → jawaban kosong.
        # Ini noise infra eval, bukan mode gagal prod (llama.cpp in-process).
        for _ in range(4):
            tr = solver.solve(str(t["id"]), t["input"])
            if tr.answer.strip():
                break
            time.sleep(2)
        # Eskalasi kini deferred (paralel di run.py); di eval cukup sinkron.
    ...
    print(f"\nwall time {total_s:.0f}s | remote: {n_remote} task, {tok_remote} token "
          f"| hasil mentah -> {OUT_PATH}")
